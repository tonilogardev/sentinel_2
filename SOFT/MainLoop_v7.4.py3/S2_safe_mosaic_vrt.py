import sys
##sys.path.insert(0, r"\\sfpcotprod01\DISC_E\AISA_python\prod_env;\\sfpcotprod01\DISC_E\AISA_python\prod_env\Library\mingw-w64\bin;\\sfpcotprod01\DISC_E\AISA_python\prod_env\Library\usr\bin;\\sfpcotprod01\DISC_E\AISA_python\prod_env\Library\bin;\\sfpcotprod01\DISC_E\AISA_python\prod_env\Scripts;\\sfpcotprod01\DISC_E\AISA_python\prod_env\bin;C:\ProgramData\Anaconda3\condabin;")
...
class s2_mosaic():
    ...
    def preprocess_safe(self):
        ...
        for ind_fuse, fuse_val in enumerate(self.fuse_vec):
            ...
            if buffer_erosion is None:
                npix_buffer = 51
                buffer_erosion = npix_buffer * self.res_out
            else:
                self.npix_buffer = round(buffer_erosion / self.res_out)
            ...
            if not os.path.exists(mosaic_vrt_f):
                ...
                for safe_ind, safe_h in enumerate(safe_h_list):
                    ...
                    safe_h.extract_real_bands(self.band_ID_vec, out_image_f=img_dict[root_name], res_out=self.img_info['dx'],
                                              bbox_dict=bbox_erosion(safe_h.img_info['10']['bbox'], buffer_erosion))
    ...
    def mosaic_gen(self):
        ...
        self.s2_mosaic_vrt = os.path.join(self.out_dir, self.capture_root_name + '_Mosaic_FUSE_{}.vrt'.format(fuse))
        self.s2_mosaoc_msk_vrt = os.path.join(self.out_dir, self.capture_root_name + '_Mask_FUSE_{}.vrt'.format(fuse))
        self.s2_mosaic_tif = os.path.join(self.out_dir, self.capture_root_name + '_Mosaic_FUSE_{}.tif'.format(fuse))
        self.s2_mosaic_msk_tif = os.path.join(self.out_dir, self.capture_root_name + '_Mask_FUSE_{}.tif'.format(fuse))
        ...
        if not os.path.exists(self.s2_mosaic_tif):
            ...
            s2_mosaic_temp_tif = os.path.join(self.out_dir, self.capture_root_name + '_Mosaic_FUSE_{}_temp.tif'.format(self.fuse))
            t00 = time.time()
            ...
            t00 = time.time()
            out_h = af.image_output_gen(s2_mosaic_temp_tif, mosaic_info['Nlines'], mosaic_info['Ncolumns'],
                                        mosaic_info['Nbands'],
            ...
            t00 = time.time()
            af.vrt2tif_bat(s2_mosaic_temp_tif, self.s2_mosaic_tif, scale_vec=[1000, 11000, 0, 10000])
            print('     * Processing time [secs] : {}'.format(round(time.time() - t00)))

            print('     * OVERALL Processing time [secs] : {}'.format(round(time.time() - t0)))

        print('\nTotal time for conversion [secs] : {}'.format(round(time.time() - t00)))
...
##    json_file=open(prm_json_f, "r")
##    proc_info = json.load(json_file)
    with open(prm_json_f) as json_file:
        proc_info = json.load(json_file)

    if out_vrt_f is None:
        out_dir = None
    else:
        print(' A specific output file name has been specified. Then, the folder is assumed to contain only SAFE (or .ZIP) files from the same FUSE. Therwise, specify the --fuse parameter to avoid overwriting output VRT')
    ...
    if bbox is None:
        img_info = {'bbox': {'xmin': None, 'xmax': None, 'ymin': None, 'ymax': None}, 'dx': img_res}
    else:
        img_info = {'bbox': bbox}

    safe_list = glob.glob(os.path.join(dir_data, '*.SAFE.zip')) + glob.glob(os.path.join(dir_data, '*.SAFE'))
    if len(safe_list) == 0:
        safe_list = glob.glob(os.path.join(dir_data, '*.zip'))

    fuse_vec = list(set([os.path.basename(_).split('_')[5][:3] for _ in safe_list]))
    capture_root_name = list(set(['_'.join(os.path.basename(_).split('_')[:5]) for _ in safe_list]))[0]
    mosaic_limits = [float(__) for __ in  [proc_info['limitsUTM'][_].split(',') for _ in proc_info['limitsUTM'] if _ in capture_root_name][0]]
    mosaic_limits_dict = {'xmin': mosaic_limits[0], 'xmax': mosaic_limits[1], 'ymax': mosaic_limits[2], 'ymin': mosaic_limits[3]}
    del safe_list

    mosaic_out_dict = {}
    mask_out_dict = {}

    for ind_fuse, fuse_val in enumerate(fuse_vec):
        safe_list = glob.glob(os.path.join(dir_data, '*{}*.SAFE.zip'.format(fuse_val))) + glob.glob(os.path.join(dir_data, '*{}*.SAFE'.format(fuse_val)))
        if len(safe_list) == 0:
            safe_list = glob.glob(os.path.join(dir_data, '*{}*.zip'.format(fuse_val)))
        print('')
        print('Input images found:')
        print('\n'.join(safe_list))
        print('')
        if out_dir is None:
            out_dir = os.path.dirname(safe_list[0])

        prefix = 'Ref_'

        if out_vrt_f is None:
            mosaic_vrt_f = os.path.join(out_dir,prefix + 'Mosaic_FUSE_{}.vrt'.format(fuse_val))

        else:
            mosaic_vrt_f = out_vrt_f
        mask_vrt_f = os.path.join(out_dir,prefix + 'Mask_FUSE_{}.vrt'.format(fuse_val))

        print(' Output mosaic: {}'.format(mosaic_vrt_f))

        if buffer_erosion is None:
            npix_buffer = 51
            buffer_erosion = npix_buffer * res_out
        else:
            npix_buffer = round(buffer_erosion/res_out)
        ...
        if not os.path.exists(mosaic_vrt_f):
            ...
            for safe_ind, safe_h in enumerate(safe_h_list):
                ...
                img_dict[root_name] = os.path.join(out_dir, root_name + '.vrt')
                safe_h.extract_real_bands(band_ID_vec, out_image_f=img_dict[root_name], res_out=img_info['dx'], bbox_dict=bbox_erosion(safe_h.img_info['10']['bbox'], buffer_erosion))
                if overview_flag:
                    print('    * Overviews being created...')
                    af.overview_gen(img_dict[root_name])
                if safe_h.producttype == 'L2A':
                    msk_dict[root_name] = safe_h.footprint_dict[msk_ref_band]
                else:
                    msk_dict[root_name] = None
                img_dict[root_name] = os.path.join(out_dir, root_name + '.vrt')
            safe_h_list = [None for _ in safe_h_list]
            ...
            if fuse is not None and fuse_val == fuse:
                mosaic_limits_vrt = [mosaic_limits_dict['xmin'],
                                     mosaic_limits_dict['ymin'],
                                     mosaic_limits_dict['xmax'],
                                     mosaic_limits_dict['ymax']]
            else:
                mosaic_limits_vrt = None
        ...
        if fuse is not None and not fuse_val == fuse:
            print('     * Tile being reprojected : {}'.format(mosaic_vrt_f))
            mosaic_fus_reproj_vrt = mosaic_vrt_f.replace(fuse_val, fuse_val+'_reprojected')
            mask_fus_reproj_vrt = mask_vrt_f.replace(fuse_val, fuse_val + '_reprojected')
            ...
            mosaic_limits_vrt = [mosaic_limits_dict['xmin'],
                                 ...
                                 mosaic_limits_dict['ymax']]
            if not os.path.exists(mosaic_fus_reproj_vrt):
                af.warp_vrt_reproj(mosaic_vrt_f, mosaic_fus_reproj_vrt, img_info['dx'], fus_epsg_dict[fuse], mosaic_limits_vrt)

            if not os.path.exists(mask_fus_reproj_vrt):
                af.warp_vrt_reproj(mask_vrt_f, mask_fus_reproj_vrt, img_info['dx'], fus_epsg_dict[fuse], mosaic_limits_vrt)

            mosaic_out_dict[fuse_val] = mosaic_fus_reproj_vrt
            mask_out_dict[fuse_val] = mask_fus_reproj_vrt

        else:
            mosaic_out_dict[fuse_val] = mosaic_vrt_f
            mask_out_dict[fuse_val] = mask_vrt_f

    s2_mosaic_vrt = os.path.join(out_dir, capture_root_name + '_Mosaic_FUSE_{}.vrt'.format(fuse))
    s2_mosaoc_msk_vrt = os.path.join(out_dir, capture_root_name + '_Mask_FUSE_{}.vrt'.format(fuse))
    s2_mosaic_tif = os.path.join(out_dir, capture_root_name + '_Mosaic_FUSE_{}.tif'.format(fuse))
    s2_mosaic_msk_tif = os.path.join(out_dir, capture_root_name + '_Mask_FUSE_{}.tif'.format(fuse))

    if not os.path.exists(s2_mosaic_vrt):
        print('     * Image list to be mosaicked : {}'.format([os.path.basename(mosaic_out_dict[_]) for _ in mosaic_out_dict]))
        af.create_vrt_file(s2_mosaic_vrt, [mosaic_out_dict[_] for _ in mosaic_out_dict], im_separate=False, im_outputBounds=bbox)

    if not os.path.exists(s2_mosaoc_msk_vrt):
        print('     * Image list to be mosaicked : {}'.format([os.path.basename(mask_out_dict[_]) for _ in mask_out_dict]))
        af.create_vrt_file(s2_mosaoc_msk_vrt, [mask_out_dict[_] for _ in mask_out_dict], im_separate=False, im_outputBounds=bbox)

    if not os.path.exists(s2_mosaic_msk_tif):
        af.vrt2tif_bat(s2_mosaoc_msk_vrt, s2_mosaic_msk_tif)

    t0 = time.time()
    print(' VRT to TIF conversion...')
    if not os.path.exists(s2_mosaic_tif):
        mosaic_info = af.get_im_info(s2_mosaic_vrt)
        s2_mosaic_temp_tif = os.path.join(out_dir, capture_root_name + '_Mosaic_FUSE_{}_temp.tif'.format(fuse))
        t00 = time.time()
        print('  Mosaic mask being eroded to avoid Sen2cor artifacts along borders...')
        msk = af.erode_binary_msk(af.read_image(s2_mosaic_msk_tif) >  0, npix_buffer)
        print('     * Processing time [secs] : {}'.format(round(time.time() - t00)))

        print('  Input value <= 1000 are set to 1001 to avoid Mosaic NoData after ESA Bias removal...')
        t00 = time.time()
        out_h = af.image_output_gen(s2_mosaic_temp_tif, mosaic_info['Nlines'], mosaic_info['Ncolumns'], mosaic_info['Nbands'],
                                    mosaic_info['gdal_datatype'],
                                    geotransform_info=mosaic_info['im_geotransformation'],
                                    src_proj_info=mosaic_info['im_geoprojection'], interleave='BAND')
        ...
        for i_band in range(int(mosaic_info['Nbands'])):
            print(f'Band being processed for ESA  offset removal : {i_band}')
            img = af.read_image(s2_mosaic_vrt, band_vec=[i_band])
            img[img <= 1000] = 1001
            img = img*msk
            af.write_im_band(out_h, img, i_band + 1)
        ...
        print('Mosaic COG generation...')
        t00 = time.time()
        af.vrt2tif_bat(s2_mosaic_temp_tif, s2_mosaic_tif, scale_vec=[1000, 11000, 0, 10000])
        print('     * Processing time [secs] : {}'.format(round(time.time() - t00)))

        print('     * OVERALL Processing time [secs] : {}'.format(round(time.time() - t0)))

    print('\nTotal time for conversion [secs] : {}'.format(round(time.time() - t00)))
    if os.path.exists(s2_mosaic_tif):
        if os.path.exists(s2_mosaic_temp_tif):
            os.remove(s2_mosaic_temp_tif)
        if del_vrt_flag:
            vrt_list = glob.glob(os.path.join(out_dir, '*.vrt'))
            for _ in vrt_list:
                print(' VTR being removed : {}'.format(_))
                os.remove(_)
...
def parse_args():
    ...
    argparser.add_argument('--fuse', type=str,
                           ...
                           default='T31')
    argparser.add_argument('--rad_flag', type=int,
                           help="For L1C data, option to convert TOA reflectance to TOA Radiance (1/0) Default=0)", default=0)
...
if __name__ == '__main__':
    ...
    main_proc(args.dir_data, args.bands, args.img_res, args.out_vrt_f, args.bbox, args.fuse, args.rad_flag, args.overview_flag, args.del_vrt_flag)
