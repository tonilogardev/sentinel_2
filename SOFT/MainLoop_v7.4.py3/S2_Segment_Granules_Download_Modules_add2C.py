...
def S2_Segment_Granules_Download_Function_add2C(Segment_2A_2B_2C,AcquisitionDate, folder_out, extractiondir, url, auth_url, username, password, poly_search, granules_orbit, Segment_Orbit, checkTwoDatastrips, granulesToCheckArea, areaForGranulesToCheck, areaThreshold, waiting_list, LastBaseline):
...
#-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    for i in range(nentries):
        ...
        if (title_element.find("S2"+Segment_2A_2B_2C+"_MSIL1C_")==0):
...
#        os.remove(filename_out)
    ...
    for i in range(nentries):
        ...
        if (title_element.find("S2"+Segment_2A_2B_2C+"_MSIL1C_")==0 and title_element[0:60] in ListOfProducts2process):
...
#                   Agafem el checksum de l'arxiu abans de descarregar-lo

                    query = urllib.parse.quote(f"Id eq '{uuid_element}'")
                    
                    result = requests.get(url + 'odata/v1/Products?$filter=' + query).json()
                    checksums = result.get("value")[0].get("Checksum")
                    
                    md5_checksum = None
                    for checksum in checksums:
                        if checksum.get("Algorithm") == "MD5":
                            md5_checksum = checksum.get("Value")

                    downloading_granule(destinationpath,uuid_element,url,access_token,md5_checksum)
                    ngranules += 1
                    end_time = time.time()
...
def downloading_granule(destinationpath,uuid_element,url,access_token,checksum_web):
    ...
    while True:
            try:
                ...
                if (file_size != 0):
                    if (checksum_web == None):
                        break
                    else:
                        if (checksum_web == md5(destinationpath)):
                            break
                        else:
...
#   Define WGS84 as CRS:
#   El footprint que proporciona la API té els punts en ordre [lon,lat]. Abans, quan el poligon s'extreia de MTD_MSIL1C.xml, els punts venien en ordre [lat,lon]
...
#   Define WGS84 as CRS:
    ...
    area_intersect = abs(area_intersect)/1000000
...
# calculk del  checksum tipus MD5 associat a un arxiu
