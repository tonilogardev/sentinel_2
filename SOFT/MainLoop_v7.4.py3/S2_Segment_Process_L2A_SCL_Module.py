...
def S2_Segment_Process_L2A_SCL(dir_in, nom_escena, Soft_Geoproces, Soft_GeoprocesGDAL, limits, UTM_Zone_to_reproject):
    ...
    try:
        if (os.path.isfile(nom_escena+".tif")):
           print ("\n","Found previous version of "+nom_escena+".tif"+". Nothing to do")
           return 0
        GSD_bandes = [20]
        NOM_bandes = ['SCL']
    ...
    #               CONVERTIR UNA BANDA EN UN .RF          
            ...
            os.rename(granule_ID+"_SCL.rf",granule_ID+".rf")
    ...
    #   FEM EL COLLAGE DE TOTS ELS GRÀNULS
        ...
        imagecollage_exe=os.path.join(Soft_Geoproces,"ICCImageCollage\exe\ICCImageCollage.exe")
        imagecollage_params=" -lfllista.txt -ol3 -ml"+limits+" "
        command = imagecollage_exe + imagecollage_params + nom_escena+".tif"
        os.system(command)    
