...
def S2_Segment_Process_NDVI(dir_in, nom_escena, Soft_Geoproces, Soft_GeoprocesGDAL, limits, UTM_Zone_to_reproject):
    ...
    try:
        if ((os.path.isfile(nom_escena+"_DEMCAT.btf")) and (os.path.isfile(nom_escena+"_NDVI.tif"))):
           print ("\n","Found previous version of "+nom_escena+"_DEMCAT.btf"+" and "+nom_escena+"_NDVI.tif"+". Nothing to do")
           return 0
    ...
    #   FEM EL COLLAGE DE TOTS ELS GRÀNULS
        ...
        os.system(Soft_Geoproces+"\ICCImageCollage\exe\ICCImageCollage.exe -lfllista.txt -ol3 -ml"+limits+" "+nom_escena+"_DEMCAT.btf\n")
        os.remove("llista.txt")
        os.remove(nom_escena+"_DEMCAT.btf.xml" )

         
        os.system(Soft_Geoproces+"\ICCImageAnalisys\exe\ICCBandCombination.exe -coefN0.,0.,0.,0.,0.,0.,0.,200.,0.,0.,0. -coefD0.,0.,0.,1.,0.,0.,0.,1.,0.,0.,0. -opt3 "+nom_escena+"_DEMCAT.btf "+nom_escena+"_NDVI16.tif\n")
...
#        os.remove(nom_escena+"_NDVI.tif.xml" )
        ...
        if S2_utis.tiff2cog(Soft_GeoprocesGDAL, dir_in, nom_escena+"_DEMCAT", "btf")!=0:
            print('\n\nError converting .btf to cog file!\n')

        if S2_utis.tiff2cog(Soft_GeoprocesGDAL, dir_in, nom_escena+"_NDVI", "tif")!=0:
            print('\n\nError converting NDVI to cog file!\n')
