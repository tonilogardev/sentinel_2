...
import S2_Segment_Granules_Download_Module as S2_Download
...
def open_xls_s2(nom_xls):
    while True:
        try:
            xlsfile = openpyxl.load_workbook(WorkingFolder+nom_xls)
...
while True:
    ...
    while True:
        ...
        Segment_2A_2B = str(sheet['B'+str(1+i)].value)
        Segment_Orbit=str(sheet['C'+str(1+i)].value)
        Segment_Downloaded=str(sheet['D'+str(1+i)].value)
        Segment_Processed_Masks=str(sheet['E'+str(1+i)].value)
...
#               Mirem si el segment està a la llista de Segments que cal esperar
                    ...
                    if (SearchProducts(SearchDateString, Days_Increment, url, username, password)==0):
                        ...
                        if (resultat_download==1):
                            ...
                            if (not must_wait):
                                ...
                                xlsfile,sheet=open_xls_s2(nom_xls)
                                sheet['D'+str(i)].value='*'
                                xlsfile.save(WorkingFolder+nom_xls)
                                something_done = 1
            ...
            elif (Segment_Processed_Masks!='*'):
                #********************************************************************************* 
                #   GENERACIÓ DE LES MÀSCARES PER CADASCUN DELS GRÀNULS
                #********************************************************************************* 

                print ('..................................................................')
                print ('              Creating masks files: ',SearchDateString)
                print ('..................................................................')
                print ("")
                sys.stdout.flush()
                ...
                try:
                    ...
                    print('\n\nSome unexpected problem when generating the granule masks!\n')
                    print ("")
                    sys.stdout.flush()
                ...
                try:
                    ...
                    for nom_file in fnmatch.filter(file_SAFE, '*.SAFE'):
                        if (nom_file[0:10]=="S2A_MSIL1C" or nom_file[0:10]=="S2B_MSIL1C"):
                            os.system(Path_L2A_ORTHO+" --resolution 10 "+Segment_Folder+"/"+nom_file+"\n")
                            shutil.rmtree(Segment_Folder+"/"+nom_file)       # eliminar producte L1C                                                     
                except:
                    print('Error when generating the L2A product!\n')
                    print ("")
                    sys.stdout.flush()
                    break
                ...
                try:
                    ...
                    for nom_file in file_SAFE:
                        if (nom_file[0:10]=="S2A_MSIL2A" or nom_file[0:10]=="S2B_MSIL2A"):
                ...
                try:
                    ...
                    nom_escena = "S2"+Segment_2A_2B+"_L2A_"+Segment_Orbit+"_"+compact_date
                    ...
                    try:
                        os.remove("S2"+Segment_2A_2B+"_L2A_"+Segment_Orbit+"_"+compact_date+".btf.xml")
                    except:
                        print("File S2"+Segment_2A_2B+"_L2A_"+Segment_Orbit+"_"+compact_date+".btf.xml  cannot be removed!\n")
                        sys.stdout.flush()
    
                    os.chdir(Segment_Folder)
                    compact_date=SearchDateString[0:4]+SearchDateString[5:7]+SearchDateString[8:10]
                    nom_escena = "S2"+Segment_2A_2B+"_SCL_"+Segment_Orbit+"_"+compact_date
                    ...
                    try:
                        os.remove("S2"+Segment_2A_2B+"_SCL_"+Segment_Orbit+"_"+compact_date+".tif.xml")
                    except:
                        print("File S2"+Segment_2A_2B+"_SCL_"+Segment_Orbit+"_"+compact_date+".tif.xml  cannot be removed!\n")
                        sys.stdout.flush()    
                ...
                try:
                    ...
                    for nom_file in file_BTF:
                        if (nom_file[0:8]=="S2A_L2A_" or nom_file[0:8]=="S2B_L2A_"):
                            
                            dir_in= Segment_Folder            
                            compact_date=SearchDateString[0:4]+SearchDateString[5:7]+SearchDateString[8:10]
                            nom_escena = "S2"+Segment_2A_2B+"_L2A_"+Segment_Orbit+"_"+compact_date
                            resultat_QuickLook_Generation = S2_QL.QuickLook_Generation(dir_in, QuickLook_dir, nom_escena, Soft_Geoproces)
                ...
                try:
                    ...
                    for nom_file in fnmatch.filter(file_SAFE, '*.SAFE'):
                        if (nom_file[0:10]=="S2A_MSIL1C" or nom_file[0:10]=="S2B_MSIL1C"):
                            ...
                            if (ier==1):
                                print('Error when executing sen2cor!\n')
                                print ("")
                                sys.stdout.flush()
                                break
                            else:                    
                ...
                try:
                    ...
                    try:
                        os.remove("S2"+Segment_2A_2B+"_L2A_"+Segment_Orbit+"_"+compact_date+"_NDVI.tif.xml")
                    except:
                        print("File S2"+Segment_2A_2B+"_L2A_"+Segment_Orbit+"_"+compact_date+"_NDVI.tif.xml  cannot be removed!\n")
                        sys.stdout.flush()
