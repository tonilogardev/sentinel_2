"""
download_sentinel_2 — L1C: descargar, verificar MD5 y extraer .SAFE

Menos es mas:
- Descarga directa desde el host de descarga (evita 401 por redirecciones)
- Token con auto-refresh y reintento 401
- Verificacion MD5 por UUID en el catalogo
- Descompresion segura de .SAFE
"""
...
def read_default_config() -> Dict[str, str]:
    return {
...
def ask_poly_menu() -> str:
    print("\nSelecciona el area de busqueda:")
    print("1. Coordenadas Catalunya")
    print("2. Coordenadas manuales (no habilitado)")
    while True:
        opt = input("Opcion [1]: ").strip() or "1"
        if opt == "1":
            return CATALUNYA_WKT
        if opt == "2":
            print("Opcion no habilitada todavia. Saliendo.")
            sys.exit(0)
...
def main():
    print("download_sentinel_2 — L1C: descargar, verificar MD5 y extraer .SAFE")
    ...
    out_dir = ask_dir("Carpeta de descargas (.zip)")
    extract_dir = ask_dir("Carpeta de extraccion (.SAFE)")
    ...
    if not items:
        print("Sin resultados para el rango/area indicados.")
        return
    ...
    if (input("\n¿Descargar y extraer todos? [s/N]: ").strip().lower() != "s"):
        print("Cancelado por el usuario.")
        return

    ok_dl = ok_md5 = ok_unzip = fail = 0
    for it in items:
        ...
        try:
            # 1) Descargar ZIP
            dest_zip = download_product(catalog, uuid, out_dir, name, tm)
            ok_dl += 1

            # 2) MD5 esperado (por UUID) y validacion local
            expected = fetch_md5_by_id(catalog, uuid)
            actual = file_md5(dest_zip)
            if expected and expected != actual:
                print(f"CHECKSUM ERROR {dest_zip.name}: esperado {expected} != calculado {actual}")
                fail += 1
                continue
            ok_md5 += 1

            # 3) Extraer .SAFE
            extracted = safe_unzip(dest_zip, extract_dir)
            ok_unzip += 1
            # 4) Borrar ZIP tras extraer correctamente
            try:
                dest_zip.unlink(missing_ok=True)
                print(f"OK: {dest_zip.name} -> {extracted} (ZIP eliminado)")
            except Exception:
                print(f"OK: {dest_zip.name} -> {extracted} (no se pudo eliminar el ZIP)")

        except Exception as e:
            fail += 1
            print(f"Fallo {name}: {e}")

    print(f"\nResumen: descargados={ok_dl} md5_ok={ok_md5} extraidos={ok_unzip} fallos={fail}")
